"""
Hybrid V4+MR Paper Trading Bot
================================
...
def fetch_candles(market: str, interval: str = "4h",
    ...
    """
    Haalt OHLCV candles op via Bitvavo publieke API.
    Retourneert DataFrame met kolommen: Open, High, Low, Close, Volume.
    ...
    # raw = [[ts_ms, open, high, low, close, volume], ...]
    # Bitvavo geeft oudste eerst terug
...
def fetch_all_candles() -> dict[str, Optional[pd.DataFrame]]:
    """Haalt candles op voor alle tickers. Logt fouten maar breekt niet."""
    result = {}
    for t in TICKERS:
...
def fetch_fng() -> dict:
    """Fear & Greed Index via alternative.me (geen API key nodig)."""
    try:
...
def last_row(df: pd.DataFrame):
    """Veilig de laatste volledige rij ophalen."""
...
def load_thresholds() -> dict:
    """
    Laad p33/p67 uit best_params_hybrid.json.
    Valt terug op 0/0 per ticker als bestand ontbreekt.
    """
    if PARAMS_FILE.exists():
        raw = json.loads(PARAMS_FILE.read_text())
        # Normaliseer: sleutel zonder -EUR
...
def portfolio_regime(regime_map: dict) -> str:
    """Meerderheidsstem van ticker regimes voor exposure check."""
...
def fetch_news(ticker: str) -> list[str]:
    ...
    coin_map = {"doge": "dogecoin", "sol": "solana", "eth": "ethereum",
                "avax": "avalanche", "ada": "cardano"}
...
def _cross_asset_label(regime_map: dict, skip_ticker: str) -> str:
    ...
    n = len(others) or 1
    if bulls / n >= 0.6:  return f"overwegend bullish ({bulls}/{n} assets)"
    if bears / n >= 0.6:  return f"overwegend bearish ({bears}/{n} assets)"
    return f"gemengd ({bulls} bull, {bears} bear, {n-bulls-bears} neutraal)"


def ask_llm(ticker: str, row, regime: str, strategy: str,
            regime_map: dict, fng: dict, headlines: list) -> dict:
    """Vraag Haiku om entry beslissing. Valt terug op KOOP bij fout."""
    fallback = {"beslissing": "KOOP", "confidence": 0.5,
                "positiegrootte": 10, "nieuws_sentiment": "neutraal",
...
{{
  ...
  "reden": "maximaal 2 zinnen"
    ...
    try:
        ...
        ps = int(parsed.get("positiegrootte", 10))
        if ps not in (7, 10, 15): ps = 10
...
def _close_position(pos: dict, close_px: float) -> tuple[float, float]:
    """Retourneer (pnl_eur, capital_terug)."""
...
def _update_trail(pos: dict, close: float, atr: float) -> dict:
    """Update trailing stop voor V4 positie. Muteert pos in-place."""
...
def check_exits(state: dict, indicators: dict) -> tuple[dict, list]:
    """Controleer trailing stops en MR exits voor open posities."""
    exits   = []
    to_del  = []
    ...
    for ticker, pos in list(state["open_positions"].items()):
        ...
        if should_exit:
            ...
            # Sla huidige waarde op voor reporting
...
def generate_entries(state: dict, indicators: dict,
    ...
    """Genereert entry signalen, filtert via LLM, voert trades uit."""
    signals    = []
    port_reg   = portfolio_regime(regime_map)
    max_expo   = EXPO_BULL_NEUT if port_reg in ("bull", "neutraal") else EXPO_BEAR
    now_str    = datetime.now(timezone.utc).isoformat()

    # Bereken totale portfolio waarde voor exposure check
    ...
    for ticker in TICKERS:
        ...
        # Positiegrootte: LLM kan overschrijven
...
def main():
    print("=" * 65)
    print(f"  Hybrid Trading Bot  —  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC")
    print("=" * 65)

    _init_clients()

    if not check_meta_config("baseline"):
        return

    # 1. Laad portfolio state
    ...
    # 2. Live OHLCV data ophalen via Bitvavo
    print("\n  2. Live data ophalen (Bitvavo) …")
    raw_frames = fetch_all_candles()

    # Controleer of alle tickers succesvol zijn
    failed = [t for t, df in raw_frames.items() if df is None]
    if len(failed) == len(TICKERS):
        print("  [FOUT] Alle Bitvavo requests mislukt — bot stopt zonder state te wijzigen.")
        sys.exit(1)
    if failed:
    ...
    port_reg = portfolio_regime(regime_map)
    print(f"     Portfolio-regime (meerderheidsstem): {port_reg}")

    # 5 & 6. Exits controleren
    print("\n  5. Exits controleren …")
